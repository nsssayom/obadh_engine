...
from .common import SPECIAL_TOKENS, iter_sentence_rows, read_json, save_manifest
...
def main() -> None:
    ...
    parser.add_argument("--output", type=Path, default=Path("data/autosuggest/models/neural/vocab.tsv"))
    parser.add_argument("--vocab-size", type=int, default=32_768)
    parser.add_argument("--min-frequency", type=int, default=3)
    args = parser.parse_args()

    report = build_vocab(args.corpus_dir, args.output, args.vocab_size, args.min_frequency)
    print(read_json(args.output.with_suffix(".manifest.json")) if False else report)
